...
nb2INLA(file.path(lap_data, "graph", fsep = ""), adj.r)
LD.graph = inla.read.graph(file.path(lap_data, "graph", fsep = ""))
...
```{r}
mesh = create_mesh(spdf = mapdf.km, plot = T)
```
...
```{r}
if(weights == "equal" ){
  Aa <- create_matrix(spdf = mapdf.km, mesh = mesh, weights = "equal")
  str(Aa[1,Aa[1,]!=0])
} else {
  Aa <- create_matrix(spdf = mapdf.km, mesh = mesh, weights = "pop.int", pop.raster = pop.dat.ng)
  str(Aa[1,Aa[1,]!=0])
}
```
